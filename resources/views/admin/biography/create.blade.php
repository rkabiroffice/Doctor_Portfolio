    ...
    <form action="{{ route('admin.biography.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid md:grid-cols-2 gap-5">
            <div class="mb-5">
                ...
                <p class="text-xs text-slate-500 mt-1">Upload MP4, WebM, MOV or AVI. Leave blank if you want to use only a YouTube link.</p>
            </div>
            <div class="mb-5"><label class="block text-sm font-medium text-slate-700 mb-1.5">YouTube URL</label><input type="url" name="youtube_url" value="{{ old('youtube_url') }}" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all duration-150"></div>
        </div>
        <div class="grid md:grid-cols-2 gap-5">
        ...
        <button class="bg-orange-600 hover:bg-orange-700 text-white font-medium px-4 py-2 rounded-lg shadow-sm transition-all duration-150">Save Biography</button>
    </form>
</div>
@endsection
